<?php
/**
 * presents a single connection to a single server
 *
 * @author Alacner Zhang <alacner@gmail.com>
 * @version $Id: Socket.php 2091 2015-09-07 02:34:18Z alacner $
 */
...
class Tattoo_Socket
{
	...
	public function __construct(
		...
		$nonWriteBuffer = FALSE
	) {
		$this->hostname = $hostname;
		$this->port = $port ? intval($port) : 80;
		...
		$this->nonWriteBuffer = $nonWriteBuffer;
	}
	...
	public function read($len)
	{
		...
		while ($len) {
			...
			if ($readable > 0) {
				...
				if ($buffer === false) {
					throw new Exception("Could not read {$len} bytes from {$this->hostname}:{$this->port}");
				} else if ($buffer == '') {
					throw new Exception("Read 0 bytes from {$this->hostname}:{$this->port}");
				}
			} else if ($readable === 0) {
				throw new Exception("Timed out reading {$len} bytes from {$this->hostname}:{$this->port} after {$this->readWriteTimeoutSec} seconds and {$this->readWriteTimeoutUsec} microseconds");
			} else {
				throw new Exception("Could not read {$len} bytes from {$this->hostname}:{$this->port}");
			}
			$data .= $buffer;
		...
		while (strlen($buf) > 0) {
			...
			if ($writable > 0) {
				...
				if ($written === -1 || $written === FALSE) {
					throw new Exception("Could not write " . strlen($buf) . " bytes to {$this->hostname}:{$this->port}");
				}
				// determine how much of the buffer is left to write
				$buf = substr($buf, $written);
			} else if ($writable === 0) {
				throw new Exception("Timed out writing " . strlen($buf) . " bytes to {$this->hostname}:{$this->port} after {$this->readWriteTimeoutSec} seconds and {$this->readWriteTimeoutUsec} microseconds");
			} else {
				throw new Exception("Could not write " . strlen($buf) . " bytes to {$this->hostname}:{$this->port}");
			}
		}
	...
	public function getSocket()
	{
		if ($this->socket === NULL) {
			...
			elseif (function_exists('pfsockopen')) {
				$this->socket = @pfsockopen($this->hostname, $this->port, $errNo, $errStr, $this->connectionTimeout);
			}
			else {
				throw new Exception('*fsockopen function were forbidden');
			}

			if (!is_resource($this->socket)) {
				throw new Exception("Could not connect to {$this->hostname}:{$this->port} ({$errStr} [{$errNo}])");
			}
