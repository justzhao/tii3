 ...
 * 
 * @author Alacner Zhang <alacner@gmail.com> 2011-12-01
 * @version $Id: Blacklist.php 488 2014-10-14 10:03:34Z alacner $
 */
class Tattoo_Blacklist
{
	/**
	 ...
	 * 
	 * $config = array(
	 *     'ip_addresses' => array('10.55.66.*'), // Forbidden IP adresses
	 *     'words' => array('Forbidden', 'keywords'), //Forbidden keywords
	 *     'regexs' => array(),//Regexs (aka. regular expression) patterns, It should be PERL style!!!
	 * );
	 * @access public
	 * @param	array  $config
	 * @var		array
	...
	public function check_ip($ips)
	{
		...
		foreach ($this->_ip_addresses as $ip) {
			...
			if (preg_match("/$regex/", $ip_address)) {
				$this->_blocked = TRUE;
				$this->log_message("Found IP address in the blacklist: '$ip'");
			}
		}
	...
	public function check_text($texts)
	{
		...
		foreach ($this->_words as $word) {
			if (stripos($this->target_text, $word) !== false) {
				$this->_blocked = TRUE;
				$this->log_message("Found forbidden word: '$word' in the given text.");
			}
		}
	...
	public function check_regex()
	{
		foreach ($this->_regexs as $regex) {
			if (preg_match($regex, $this->target_text, $m)) {
				$this->_blocked = TRUE;
				$this->log_message("Found forbidden word '$regex' in the text.");
			}
		}
