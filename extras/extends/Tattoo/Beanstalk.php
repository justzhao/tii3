 ...
 * Under Beanstalk Protocol 1.4.6,1.6
 * More information is available at https://github.com/kr/beanstalkd
 * 
 * @author  Alacner Zhang <alacner@gmail.com>
 * @version $Id: Beanstalk.php 5027 2016-05-09 12:03:49Z alacner $
 */
class Tattoo_Beanstalk
{
	...
	const LOWER_PRIORITY = 2048;

	const DEFAULT_DELAY = 0;
	const DEFAULT_TTR = 60; //time-to-run
	...
	{
		is_null($config) && $config = 'default';
		is_string($config) && $config = Desire_Config::get("tattoo.beanstalk.$config", array());
	...
	protected function connect()
	{
		...
		list($sec, $usec) = explode('.', $this->config['timeout'] . '.');
	...
	{
		is_resource($this->socket) || $this->socket = $this->connect();
		return $this->socket;
	}
	...
	/**
	 * When a tube is cleared, its contents is droped.
	 * 
	 * @param string $tube is a name at most 200 bytes. It specifies a tube to add to the watch
	 *     list. If the tube doesn't exist, it will be created.
	 */
	public function clear_tube($tube)
	{
		$tube = $this->_tube($tube);
		try {
			$line = $this->_command("clear-tube $tube\r\n");
			switch ($line) {
				case 'CLEARED': return true; break;
				case 'NOT_FOUND': return false; break;
				default:
			}
			return false;
			...
			$this->watch($tube);
			while($reserve = $this->reserve_with_timeout(0)) {
				$this->delete($reserve['id']);
			}
			$this->ignore($tube);

			$this->use_tube($tube);
			while($reserve = $this->peek_delayed()) {
				$this->delete($reserve['id']);
			}
			while($reserve = $this->peek_buried()) {
				$this->delete($reserve['id']);
			}
			return true;
		}
	 ...
	 */
	public function pause_tube($tube, $delay = self::DEFAULT_DELAY)
	{
		$tube = $this->_tube($tube);
	...
	/**
	 * Simple to parser beanstalk return infomation.
	 * @param string $yaml
	 * @return array|bool
	 */
	public function yaml_parser($yaml = '---')
	{
		$lines = explode("\n", trim($yaml));
		$doc = array_shift($lines);
		if ($doc !== '---') return array();
		$return = array();
