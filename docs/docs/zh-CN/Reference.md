...
Desire 是一套给 PHP 开发者使用的开发框架和工具包，一直秉承简洁实用的设计原则，在保持出色的性能和至简的代码的同时，也注重易用性，拥有众多的功能和特性。 她是一个小巧但功能强大的单文件 PHP 框架。他总大小不过200k，功能却极其强大，可同时支持编写网页、服务器脚本、高性能socket服务器等。


| 版本 | 说明 | 贡献者 |
| ------|------|----- |
| [Desire-2.16.0223.1627](https://raw.githubusercontent.com/alacner/desire/master/build/Desire-2.16.0223.1627.php) | 稳定版本 | @alacner |

地址 [http://desire.triones.org]() 欢迎关注
...
``` php
<?php
require_once __DIR__ . '/../library/Desire-2.16.0223.1627.php';//加载 desire 打包库
Desire_Config::setDir(__DIR__);// 修改配置文件的路径
```
...
主要提供了一个重要的类：Desire
可以利用 Desire::object方法来创建单例，call的参数可以参考 call_user_func 参数，第一个是class，其他为传入__construct的参数；
为了便于简化调用单例模式的对象
同时支持多语言报错，也可以自定义报错。
``` php
		Desire::validator($params, array(
...
```php
Desire_ConfigWrapper
{
	public Desire_ConfigWrapper function __construct([mixed $args [, mixed $... ]])
}
```
### Desire_ConfigWrapper 类
...
`Desire_ConfigWrapper __construct([mixed $args [, mixed $... ]])`

构造一个新的 Desire_ConfigWrapper 对象。
...
$a = new Desire_ConfigWrapper(['key' => 'value', 'arr' => ['abc' => 'xyz', 'x' => 1]], $obj, new ClassName);
...
注释字符串，格式请移步 `Desire_DocCommentParser 类介绍`：
...
| ------------- |:-------------:| -----:|
| enabled      |  true | 是否启用|
| cacheName	 | method.{0}.{1.userid} | 默认值为入参的哈希值，支持通过`Desire::render`的方式自定义唯一值 |
...
Desire_Delegate
{
	public Desire_Delegate function __construct(mixed $that)
...
`Desire_Delegate __construct(mixed $that)`

构造一个新的 Desire_Delegate 对象。
...
$a = new Desire_Delegate(new ClassName);
...
```php
Desire_DocCommentParser
{
	public Desire_DocCommentParser function __construct(string $commentString)
...
`Desire_DocCommentParser __construct(string $commentString)`
...
$a = new Desire_DocCommentParser($doc);
...
``` php
<?php
throw new Desire_Exception("hello %s", 'world');
```
...
`bool Desire_Exception::set_error_handler(int $errno , string $errstr [, string $errfile [, int $errline [, array $errcontext ]]])`
...
`bool Desire_Exception::set_error_exception_handler(int $errno , string $errstr [, string $errfile [, int $errline [, array $errcontext ]]])`
...
`bool Desire_Exception::set_exception_handler(Exception $exception)`
...
```php
<?php
set_exception_handler('Desire_Exception::set_exception_handler');
...
Desire_Filesystem
{
	...
	public static string function tempnam([string $prefix = 'desire.'[, string $suffix = '']])
	public static string function tempfile([mixed $args [, mixed $... ]])
	public static string function hashfile(string $key[, string $namespace = 'desire'[, string $suffix = ''[, string $path = NULL]]])
...
`mixed Desire_Filesystem::getTempDir( void )`
...
``` php
<?php
echo Desire_Filesystem::getTempDir();///var/folders/y_/spjx8212m00n/T/cli
```
...
`mixed Desire_Filesystem::getDataDir( void )`
...
``` php
<?php
echo Desire_Filesystem::getDataDir();///desire/data/cli
```
...
`object Desire_Http::post(string $url[, mixed $data = ''[, array $headers = [][, array $options = [][, string $method = 'POST']]]])`
...
> 字符串数据或者`Desire_Http::preparePostBody()`的参数
...
``` php
<?php
print_r(Desire_Http::post('http://localhost/test.php'));
/**
stdClass Object
(
    [state] => 200
...
(
    ...
    [HTTP_USER_AGENT] => Desire/2.16.0223.1627
...
`object Desire_Http::get(string $url[, array $headers = [][, array $options = [][, string $method = 'GET']]])`
...
返回object，选项数据，参考 `Desire_Http::post()`
...
``` php
<?php
print_r(Desire_Http::get('http://localhost/test.php'));
/**
stdClass Object
(
    [state] => 200
...
`string Desire_Http::streamBuilder(string $protocol[, array $headers = [][, string $data = '']])`
...
print_r(Desire_Http::streamBuilder('GET /test.php HTTP/1.1', ['foo' => 'bar'], 'body'));
/**
GET /test.php HTTP/1.1
User-Agent: Desire/2.16.0223.1627
Content-Length: 4
Connection: close
foo: bar
...
`array Desire_Http::parseUrl(string $url)`
...
print_r(Desire_Http::parseUrl($url));
/**
Array
(
    [scheme] => http
...
`array Desire_Http::preparePostBody(array $formvars[, array $formfiles = []])`
...
``` php
<?php
print_r(Desire_Http::preparePostBody(['foo' => 'bar'], ['test' => 'test.txt']));
/**
Array
(
...
`array Desire_Http::parser(string $raw[, string $remoteAddr = '127.0.0.1'[, int $remotePort = 0]])`
...
还原 `Desire_Http::streamBuilder`组装的HTTP协议数据。
...
``` php
<?php
$body = Desire_Http::streamBuilder('GET /test.php HTTP/1.1', ['foo' => 'bar'], 'body');
print_r(Desire_Http::parser($body, '127.0.0.1', 1234));
/**
Array
(
    [0] => Array
        (
            ...
            [REQUEST_URI] => /test.php
            [SERVER_PROTOCOL] => HTTP/1.1
            [SERVER_SOFTWARE] => Desire/2.16.0223.1627
            [SERVER_NAME] => Desire.Triones
            [HTTP_HOST] => 
            [HTTP_USER_AGENT] => Desire/2.16.0223.1627
            [HTTP_ACCEPT] => 
...
`array Desire_Http::parseCookie(string $cookie)`
...
``` php
<?php
print_r(Desire_Http::parseCookie('LAST_LANG=zh; expires=Sat, 14-Oct-2017 14:14:35 GMT; Max-Age=31536000; path=/; domain=.php.net'));
/**
Array
(
    [LAST_LANG] => zh
...
`string Desire_Http::tidyCookie([mixed $args [, mixed $... ]])`
...
``` php
<?php
print_r(Desire_Http::tidyCookie('LAST_LANG=zh; expires=Sat, 14-Oct-2017 14:14:35 GMT;','Max-Age=31536000; path=/; domain=.php.net'));
...
`string Desire_Http::urlAppend(string $url[, mixed $append = NULL])`
...
``` php
<?php
print_r(Desire_Http::urlAppend('http://host.name', 'foo=bar'));
//http://host.name?foo=bar

print_r(Desire_Http::urlAppend('http://host.name?abc=xyz', 'foo=bar'));
...
`string Desire_Http::concat([mixed $args [, mixed $... ]])`
...
``` php
<?php
echo Desire_Http::concat('http://host.name/', '/a/b/c', '/d/e/f');
...
``` php
<?php
echo Desire_Http:: getMimeType('html');//text/html
```
...
`string Desire_Http::getHttpStatus([int $code = 200])`
...
``` php
<?php
echo Desire_Http:: getHttpStatus();//OK
echo Desire_Http:: getHttpStatus(200);//OK
echo Desire_Http:: getHttpStatus(302);//Moved Temporarily
echo Desire_Http:: getHttpStatus(400);//Bad Request
echo Desire_Http:: getHttpStatus(404);//Not Found
```
...
`Desire_Event::filter('desire.http.options', $options)`
...
Desire_Http::get('https://www.host.com', [], $options);
```	
...
通过更改 `remote_socket`来达到移（通）花（过）接（代）木（理）来访问目标请求的目的。
 ...
 + 全局批量指定
	...
	```
	Desire_Event::register('desire.http.options', function($options)
	{
...
* desire.logger.handler 默认为：Desire_Logger_File
* desire.logger.priority 默认为：Desire_Logger_Constant::ERR


### 预定义常量

`Desire_Logger_Constant`
...
### 类摘要
```php
Desire_Logger_Abstract
{
	public void setPriority([int $priority = Desire_Logger_Constant::ERR])
	public int function getPriority(void)
	public string function getPriorityName([int $priority])
	abstract public int function doLog(string $message[, int $priority = Desire_Logger_Constant::ERR[, mixed $extras = NULL]]);
}
```
### Desire\_Logger\_Abstract 抽象类
...
`void setPriority([int $priority = Desire_Logger_Constant::ERR])`

设置处理日志级别，默认：`Desire_Logger_Constant::ERR`
...
`public int function doLog(string $message[, int $priority = Desire_Logger_Constant::ERR[, mixed $extras = NULL]]);
`
...
path
>日志输出路径，默认为缓存目录。参考：`Desire_Filesystem::getTempDir`

format
>文件名生成格式， 默认：Ymd。参考：`Desire_Time::format`

##### 返回值

无返回值

### Desire\_Logger\_Dao 类
...
`int Desire_Math::getSequenceId ( void )`
...
``` php
<?php
echo Desire_Math::getSequenceId();//1
echo Desire_Math::getSequenceId();//2
```
...
`float Desire_Math::getSystemBits ( void )`
...
`int Desire_Math::getStatus(int $status, int $bit[, int $span = 1])`
...
<?php
$status = 5;
echo Desire_Math::getStatus($status, 1);//1
echo Desire_Math::getStatus($status, 2);//0
echo Desire_Math::getStatus($status, 3);//1
```
...
`mixed Desire_Math::setStatus(int $status, int $bit[, mixed $val = true[, int $span = 1]])`
...
<?php
$status = 0;
echo $status = Desire_Math::setStatus($status, 1, true);//1
echo $status = Desire_Math::setStatus($status, 2, false);//1
echo $status = Desire_Math::setStatus($status, 3, true);//5
```
...
`int  Desire_Math::getFloatLength(float $num)`
...
``` php
<?php
echo Desire_Math::getFloatLength(0.123456);//6
```
##### 参见
...
`bool Desire_Math::pather(array &$chain, array $arr, string $n, string $v, string $c[, bool $o = true])`
...
<?php
$chain = [];
Desire_Math::pather($chain, [['id' => 1, 'key' => 'founder', 'childKey'=>[['id' => 11, 'key' => 'me', 'childKey'=>[]]]]], 'me', 'key', 'childKey');
print_r($chain);
/**
Array
(
...
$chain = [];
Desire_Math::pather($chain, [['id' => 1, 'key' => 'founder', 'childKey'=>[['id' => 11, 'key' => 'me', 'childKey'=>[]]]]], 'me', 'key', 'childKey', false);
print_r($chain);
/**
Array
(
...
`mixed Desire_Math::toGuidString(mixed $mix[, callable $func = 'md5'])`
...
``` php
	echo Desire_Math::toGuidString('desire');//dae63fb4ce0398632fc27357e9cba450
```
##### 参见
...
`string Desire_Math::random([int $len = 6[, string $str1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz'[, string $str2 = '']]])`
...
Example #1 返回默认随机字符串
``` php
	echo Desire_Math::random();//ppuJR8
```
##### 参见
...
`string Desire_Math::randomReadableString([int $len = 6])`
...
Example #1 返回默认随机字符串
``` php
	echo Desire_Math::randomReadableString();//pucugu
```
...
`string Desire_Math::randomDigital([int $len = 6])`
...
Example #1 用md5做混淆函数返回唯一值
``` php
	echo Desire_Math:: randomDigital();//821456
```
##### 参见
...
`string Desire_Math::mask(string $str, array $indexes[, string $mask = '*'])`
...
echo Desire_Math::mask($email, range(3, (strpos($email, '@')-1)));//ala****@gmail.com
echo Desire_Math::mask('12312345678', range(3, 6));;//123****5678
```

Example #2 唯一码生成器
``` php
echo Desire_Math::mask(Desire_Math::random(32), [8, 13, 18, 23], '-');//XembMRna-QZlh-J368-1nT0-fURVuIiI
```
##### 参见
...
`string Desire_Math::guid([string $chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz'])`
...
Example #1 英文+数字的字符串
``` php
	echo Desire_Math::guid();//XembMRna-QZlh-J368-1nT0-fURVuIiI
```
##### 参见
...
Example #1
``` php
	echo Desire_Math:: guid16();//cf814149-cf73-9b2a-d8d7-f35283c7
```
##### 参见
...
`mixed Desire_Math::getScaleRandom(array $scales[, array $weight = []])`
...
$result = [];
for($i = 0; $i < 1000; $i++) {
        $result[Desire_Math::getScaleRandom(['一等奖' => 10, '二等奖' => 20, '三等奖' => 70])]++;
}
print_r($result);
/**
Array
...
$result = [];
for($i = 0; $i < 1000; $i++) {
        $result[Desire_Math::getScaleRandom(['192.168.1.100' => 100, '192.168.1.101' => 200, '192.168.1.102' => 50])]++;
}
print_r($result);
/**
Array
...
`string Desire_Math::hash(string $key[, array $nodes = [][, int $replicas = 32]])`
...
Example #1 分布式缓存服务器
``` php
	echo Desire_Math::hash('cache_key', ['192.168.1.1', '192.168.1.2', '192.168.1.3']);//192.168.1.1
	echo Desire_Math::hash('change_cache_key', ['192.168.1.1', '192.168.1.2', '192.168.1.3']);//192.168.1.2
```
##### 参见
...
`string Desire_Math::decst(int $number)`
...
``` php
	echo Desire_Math::decst(1234567890);//1LY7VK
```
##### 参见
...
`int Desire_Math::stdec(string $str)`
...
Example #1 64进制转十进制
``` php
	echo Desire_Math::stdec('1LY7VK');//1234567890
```
##### 参见
...
`string Desire_Math::uniqId([string $prefix = ''])`
...
Example #1 唯一值生成器
``` php
echo Desire_Math::uniqId();//1bslnCazHO
echo Desire_Math::uniqId('prev');//prev1bslnCazUc
```
##### 参见
...
`mixed Desire_Math::hashStr(string $str[, bool $decst = true])`
...
Example #1 用md5做混淆函数返回唯一值
``` php
	echo Desire_Math::hashStr('desire');//1zwDRE
	echo Desire_Math::hashStr('desire', false);//1831364012
```
##### 参见
...
`mixed Desire_Math::hashArr([mixed $args [, mixed $... ]])`
...
Example #1 生成哈希
``` php
echo Desire_Math::hashArr('desire');//c3c84bf0c498946f97959101e536652e6353eb59
echo Desire_Math::hashArr('desire', 'is', 'usefull');//c69a2f9480d2a38ebcaf6250a9d6512c600470cb
echo Desire_Math::hashArr('desire', 'usefull', 'is');//c69a2f9480d2a38ebcaf6250a9d6512c600470cb
```
##### 参见
...
`int Desire_Network::getAvailablePort([int $start = 10000[, int $end = 20000]])`
...
``` php
<?php
echo Desire_Network::getAvailablePort();//10000
echo Desire_Network::getAvailablePort(3456, 5678);//3456
```
...
`string Desire_Network::getIp(void)`
...
``` php
<?php
echo Desire_Network::getIp();//192.168.1.101
```
...
`string Desire_Network::getMacAddr(void)`
...
``` php
<?php
echo Desire_Network::getMacAddr();//00:1c:40:20:10:08
```
...
`array Desire_Network::getMacAddrs(void)`
...
``` php
<?php
print_r(Desire_Network::getMacAddrs());
/**
Array
(
	[0] => 00:1c:40:20:10:08
...
`array Desire_Network::parseHost(string $host[, $port = 22])`
...
``` php
<?php
print_r(Desire_Network::parseHost('192.168.1.2:1234', 3456));
/**
Array
(
	[0] => 192.168.1.2
...
print_r(Desire_Network::parseHost('12.34.56.78', 3456));
/**
Array
(
	[0] => 12.34.56.78
...
`int Desire_Network::getPort(string $host[, int $port = 22])`
...
``` php
<?php
echo Desire_Network::getPort('192.168.1.2:1234', 3456);//1234
echo Desire_Network::getPort('12.34.56.78', 3456);//3456
```
...
`array Desire_Network::parseIpRange(string $range)`
...
``` php
<?php
print_r(Desire_Network::parseIpRange('1.2.3.*'));
/**
Array
(
	[0] => 1.2.3.0
...
`bool Desire_Network::ipInRange(string $ip, string $range)`
...
``` php
<?php
echo Desire_Network::ipInRange('1.2.3.4', '1.2.3.*');//true
```
...
`bool Desire_Network::ipInRanges(string $ip[, array $rules = [][, bool $default = true]])`
...
``` php
<?php
echo Desire_Network::ipInRanges('1.2.3.4', ['1.2.3.*' => false], true);//false
```
...
### 简介
自动利用`Desire::object`加载子目录下的同级类。

静态类加载器，只需继承 `Desire_Service` 类，如：

```php
<?php
class ClassName_Service extends Desire_Service {}
```
...
* `ClassName_Service::foo($arg1, $arg2...)->bar(...);`
	...
	> 方法名小写，实例化这个类的时候，会自动采用`Desire_Delegate`代理类方式，这样被调用的方法都会解析注释，如：自动增加缓存等，参考：`Desire_Delegate`章节
* `ClassName_Service::Foo($arg1, $arg2...)->bar(...);`
...
`bool Desire_Time::timezone([string $timezone = 'UTC'])`
...
``` php
<?php
Desire_Time::timezone('Aisa/Shanghai');
$time = strtotime("2012-07-05 16:43:21");//1341477801
Desire_Time::timezone('UTC'); 
...
``` php
<?php
echo Desire_Time::micro();//1476109725.7075
```
...
`float Desire_Time::milli(void)`

在Desire_Time::micro()扩大了1000倍并取整
...
``` php
<?php
echo Desire_Time::milli();//1476109725707
```
...
`int Desire_Time::now([bool $online = false])`
...
``` php
<?php
echo Desire_Time::now();//1476109725
```

##### 参考

`int Desire_Time::online(void)` `int Desire_Time::locale(void)`
...
默认从东区的["time.nist.gov", "time-d.nist.gov"]获取NTP服务器时间，如果需要添加更多服务器，可以通过配置配置项：'desire.time.servers'来增加。
...
``` php
<?php
echo Desire_Time::online();//1476109725
```
...
``` php
<?php
echo Desire_Time::locale();//1476109725
```
...
`int Desire_Time::totime([mixed $time = NULL[, mixed $now = NULL]])`
...
``` php
<?php
echo Desire_Time::totime("now");//1476110816
echo Desire_Time::totime("10 September 2000");//968515200
echo Desire_Time::totime("+1 day");//1476197216
echo Desire_Time::totime("+1 week");//1476715616
echo Desire_Time::totime("+1 week 2 days 4 hours 2 seconds");//1476902818
echo Desire_Time::totime("next Thursday");//1476288000
echo Desire_Time::totime("last Monday");//1475424000
echo Desire_Time::totime("last Monday", "+1 day");//1476028800
```
...
`string Desire_Time::format([string $format = 'Y-m-d H:i:s'[, mixed $time = NULL[, mixed $now = NULL]]])	`
...
``` php
<?php
echo Desire_Time::format();//"2012-07-05 16:43:21"
echo Desire_Time::format('l jS \of F Y h:i:s A');//Monday 8th of August 2005 03:12:46 PM
echo Desire_Time::format('l jS \of F Y h:i:s A', "+1 day");//Tuesday 9th of August 2005 03:12:46 PM
```
...
`int Desire_Time::formatted(string $format[, mixed $time = NULL[, mixed $now = NULL]])`

可用于计算整点时间戳，如，当前小时：`Desire_Time::formatted('Y-m-d H:00:00')`

##### 参数

与Desire_Time::format()同
...
``` php
<?php
echo Desire_Time::formatted('Y-m-d H:00:00');//1476115200
```
...
`int Desire_Time::interval(mixed $time1[, mixed $time2 = NULL])`
...
``` php
<?php
echo Desire_Time::interval("-1 day");//86400
echo Desire_Time::interval("+1 day", "+2 days");//86400
```
...
`bool Desire_Time::inSeconds(mixed $time1[, int $forward = -60[, int $afterward = 60[, mixed $time2 = NULL]]])`
...
``` php
<?php
echo Desire_Time::inSeconds("+1 days");//false
echo Desire_Time::inSeconds("+1 seconds");//true
```
...
`string Desire_Time::readable(float $microtime[, string $format = '%.3f%s'[, int $round = 3]])`
...
``` php
<?php
echo Desire_Time::readable(0.032);//32ms
echo Desire_Time::readable(110.032);//110.032s
```
...
`int Desire_Time::nexttime([string $cron = '* * * * *'[, mixed $time = NULL[, mixed $now = NULL]]])`
...
``` php
<?php
echo Desire_Time::nexttime();//1476117600
echo Desire_Time::nexttime("* 1 * * *");//1476118800
```
...
>开发者可以很方便的开发出自己的协议，为了方便使用，我们提供了websocket(ws)，http协议的实现。实现自己的协议后，只需要注册到`desire.worker.protocols`就可以使用这个协议了。。。
...
----
### Tattoo
Tattoo扩展类，主要是对Desire的补充，将Desire并不常用的类和方法剥离出来，让Desire可以很方便的打成一个文件

FAQ：常见问题
----
```php
<?php define('CONFIGURATION_DIRECTORY', dirname(__FILE__));
require_once CONFIGURATION_DIRECTORY . '/../../../build/Desire-0.4.8.1445.Packer.php';// 加载 desire 库 //require_once CONFIGURATION_DIRECTORY . '/../../../library/Bootstrap.php';// 加载 desire 库
/* 添加一些系统路径 */ Desire_Config::set(); 详细见 library .configs 目录中的desire.config.php
Desire_Config::setDir(DESIRE_DIRECTORY . '/.configs'); //默认配置文件存放目录为框架目录的.config目录，如果么有正常设置，不能正常运行框架。
唯一定义了一个常量：DESIRE_DIRECTORY 框架的路径
Desire_Event::filter('desire.application.processor.http.getView'
require_once Desire_Config::get('library_dir') . '/Bootstrap.php'; //加载Blaze引导文件
Desire_Application_Session::start();//开启session
Desire_Controller_Front::run(Desire_Config::get('application_dir'));
...
//session start if (Desire_Config::get('desire.application.session.start', false)) {
    Desire_Application_Session::start(Desire_Config::get('desire.application.session.handler', null));
}
Desire_Application::run();
基础 *************
可以利用 Desire::object方法来创建单例，call的参数可以参考 call_user_func 参数，第一个是class，其他为传入__construct的参数； 为了便于简化调用单例模式的对象，可以都放在 Blaze_Factory 中，比如：
	/**
	 * @return Desire_Cache
	 */
...
__config_dir__
desire.debug_mode desire.timezone desire.library.include desire.library.* path/to/some/class lang-*[default].*
desire.auth_code_key md5(__FILE__)
desire.application.directory desire.application.rewrite.http array() preg_replace desire.application.rewrite.cli //Is not implemented
	const CONFIG_MODULE = 'desire.application.module';
	const CONFIG_CONTROLLER = 'desire.application.controller';
	const CONFIG_ACTION = 'desire.application.action';
		$path = Desire_Config::get('desire.controller.cookie.path', '/');
		$domain = Desire_Config::get('desire.controller.cookie.domain', null);
		$secure = Desire_Config::get('desire.controller.cookie.secure', false);
		$httponly = Desire_Config::get('desire.controller.cookie.httponly', false);
desire.logger.priority desire.logger.handler
desire.cache.chain array('memcache', 'apc', 'file') desire.cache.memcache' desire.cache.file', Desire_Config::get('desire.temp_dir', sys_get_temp_dir())
desire.database.*[default] array(//default config
...
Desire_Application_Processor_Abstract get(.*)Helper/
创建session：
可以使用下面2种方式： Desire_Application_Session::start();
or
Desire_Application_Session::start('Desire_Application_SessionHandler_Cache');
Logger：
继承 Desire_Logger_Abstract 实现 Blaze_Logger
DAO：
Desire_Dao
默认是desire.config.php中的 database 的值 将继承创建对象，更改成配置生成模式，如果需要单例，可以用 Desire::object 的方法调用。
Cache： 默认是desire.config.php中的 cache 的值 将继承创建对象，更改成配置生成模式，如果需要单例，可以用 Desire::object 的方法调用。
Desire_Cache
配置：
array(
    memcache => array(
	...
	'cache' => array(
		...
		'file' => array(
			'prefix' => 'blaze',
			//'directory' => Desire_Config::get(_temp_dir_),
		),
		//'locker' => Desire_Config::get(_temp_dir_),
	),
如果需要使用过滤器： 继承 Desire_Controller_Filter_Abstract 实现 Blaze_Controller_Filter_xxx
需要创建助手：
继承 Desire_Controller_Helper_Abstract 实现 Blaze_Controller_Helper_Something
则在view中调用
$this->getController()->getBlazeHelper()->something()->method(); Desire中的基础助手csrf则可以： $this->getController()->getHelper()->csrf()->getName(); $this->getController()->getHelper()->csrf()->getValue(); $this->getController()->getHelper()->csrf()->getInput();
view的安全输出： $this->output($now);//html的防xss $this->output($var, 'javascript');//javascript的防xss
========================
如果需要定制 Router 继承 Desire_Controller_Router_Abstruct
扩展 *************
当设置好： Desire_Config::set('__extend_library_dir', dirname(__FILE__) . '/../../../extras/extends'); // desire extend library
会自动在__extend_library_dir目录加载EDesire_*的类
验证码： 继承 EDesire_Captcha_Abstract
/application/$pro/filters/beforeFilter.php 变量 Task_beforeFilter
	/**
	 * @return Desire_Application_Controller_Abstract
	 */
...
### desire.config.php
```
EVENT: desire.application.event |- router |- controller |- action |- view |- response
CONFIG: desire.debug_mode => true, //开启debug模式 desire.logger_priority => Desire_Logger_Constant::ALL, //日志的等级 desire.temp_dir => sys_get_temp_dir(), //临时文件路径 **
desire.application |- directory => dirname(__DIR__) . '/application', |- module => default |- controller => index |- action => index |- parameter.xxxxx |- context //入口？
|- cookie
	|- path
	|- domain
	|- secure
...
|- modules
	|- default =>
		|- directory => desire.application.directory + default
desire.controller.runtime |- router |- dispatcher |- action |- view |- response
desire.database => array(
	'dsn' => array(
	'host' => 'localhost',
	'port' => 3306,
	'dbname' => 'dbname',
...
* 全面优化加载机制，增加Tattoo附属框架，用于迁移Desire非常用功能
...
[Desire PHP框架]: #desire-php-framework
[序言]: #preface
[经典案例]: #projects
[例子]: #examples
[版权申明]: #copyright
...
[关于本手册]: #about
[附录]: #appendix
[desire.config.php]: #desire-config-php
[更新日志]: #change-log
[鸣谢]: #thanks
