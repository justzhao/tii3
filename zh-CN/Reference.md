...
`void Tii_Worker_Timer::signalHandler(void)`
...
$task->onWorkerStart = function($task)
{
    ...
    // 10秒后发送一次邮件
    // 定时调用带命名空间的类的静态方法
    Tii_Worker_Timer::add(10, array('\Task\Mail', 'send'), array($to, $content), false);
};
...
Example #7 定时器中销毁当前定时器（use闭包方式传递`$timer_id`）
...
$task->onWorkerStart = function($task)
{
    ...
    $timer_id = Tii_Worker_Timer::add(1, function()use(&$timer_id, &$count)
    {
        echo "Timer run $count\n";
        // 运行10次后销毁当前定时器
        if($count++ >= 10)
        {
            echo "Tii_Worker_Timer:: delete($timer_id)\n";
...
Example #8 定时器中销毁当前定时器（参数方式传递`$timer_id`）
...
$task->onWorkerStart = function($task)
{
    $mail = new Mail();
    // 要想$timer_id能正确传递到回调函数内部，$timer_id前面必须加地址符 &
    $timer_id = Tii_Worker_Timer::add(1, array($mail, 'send'), array('to', 'content', &$timer_id));
};
...
Example #9 只在指定进程中设置定时器

``` php
<?php
//一个worker实例有4个进程，只在id编号为0的进程上设置定时器。
$task = new Tii_Worker('', ['count' => 4]);
$task->onWorkerStart = function($task)
{
    // 只在id编号为0的进程上设置定时器，其它1、2、3号进程不设置定时器
    if($worker->id === 0)
    {
        Tii_Worker_Timer::add(1, function(){
...
`bool Tii_Worker_Timer::delete(mixed $fd)`
...
{
    // 每2秒运行一次
    $timer_id = Tii_Worker_Timer::add(2, function()
    {
        echo "task run\n";
    });
    // 20秒后运行一个一次性任务，删除2秒一次的定时任务
    Tii_Worker_Timer::add(20, function($timer_id)
    {
        Tii_Worker_Timer::delete($timer_id);
...
$task->onWorkerStart = function($task)
{
    ...
    $timer_id = Tii_Worker_Timer::add(1, function()use(&$timer_id)
    {
        static $i = 0;
        echo $i++."\n";
        // 运行10次后删除定时器
...
`void Tii_Worker_Timer::delAll(void)`
...
的顺序尝试性加载扩展，找到已经安装的扩展后，实例化事件处理器供定时器调用。
...
参见[特性]，需要基于这些协议通讯时可以直接使用，使用方法及时在初始化时指定协议，例如

```
// websocket://0.0.0.0:2345 表明用websocket协议监听2345端口
$websocket_worker = new Tii_Worker('websocket://0.0.0.0:2345');

// text协议
$text_worker = new Tii_Worker('text://0.0.0.0:2346');

// chunk协议
$chunk_worker = new Tii_Worker('chunk://0.0.0.0:2347');

// tcp Worker，直接基于socket传输，不使用任何应用层协议
$tcp_worker = new Tii_Worker('tcp://0.0.0.0:2348');

// udp Worker，不使用任何应用层协议
$udp_worker = new Tii_Worker('udp://0.0.0.0:2349');

// unix domain Worker，不使用任何应用层协议
$unix_worker = new Tii_Worker('unix:///tmp/tii.sock');

```

#### 使用自定义的通讯协议

当自带的通讯协议满足不了开发需求时，开发者可以定制自己的通讯协议。

提示：

内置了一个text协议，协议格式为文本+换行符。text协议开发调试都非常简单，可用于绝大多数自定义协议的场景，并且支持telnet调试。如果开发者要开发自己的应用协议，可以直接使用text协议，不用再单独开发。
